export default class EbookGenerator {
    ...
    static createPageXHTML( pageName, size ) {
        let content = '';
        content += '<?xml version="1.0" encoding="utf-8"?>\n';
        ...
        content += '<head>\n';
        if( size ) {
            content += '    <meta name="viewport" content="width=' + size.width + ', height=' + size.height + '"/>\n';
        }
        ...
        content += '    <div>\n';
        if( size ) {
            content += '        <img alt="' + pageName + '" src="../img/' + pageName + '" width="' + size.width + '" height="' + size.height + '"/>\n';
        } else {
    ...
    static createContentOPF( uid, title, pages, options = {} ) {
        let language = options.language || 'und';
        let direction = options.direction === 'rtl' ? 'rtl' : 'ltr';
        let content = '';
        content += '<?xml version="1.0" encoding="UTF-8"?>\n';
        ...
        if( options.kindleComic ) {
            // Kindle fixed-layout comic metadata (as generated by Kindle Comic Converter),
            // preserved by Amazon's Send to Kindle EPUB conversion
            content += '        <meta name="fixed-layout" content="true"/>\n';
            content += '        <meta name="original-resolution" content="' + options.kindleComic.width + 'x' + options.kindleComic.height + '"/>\n';
            content += '        <meta name="book-type" content="comic"/>\n';
            content += '        <meta name="primary-writing-mode" content="' + ( direction === 'rtl' ? 'horizontal-rl' : 'horizontal-lr' ) + '"/>\n';
            content += '        <meta name="zero-gutter" content="true"/>\n';
            content += '        <meta name="zero-margin" content="true"/>\n';
            content += '        <meta name="region-mag" content="false"/>\n';
            content += '        <meta name="cover" content="IMG_0"/>\n';
        }
        content += '    </metadata>\n';
        content += '    <manifest>\n';
        ...
        content += '    </manifest>\n';
        content += '    <spine toc="ncx" page-progression-direction="' + direction + '">\n';
        pages.forEach( ( page, index ) => {
